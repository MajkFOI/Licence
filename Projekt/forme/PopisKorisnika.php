<?php
    ...
    if(isset($_GET['korisnikID'])){
        ...
        $upit = "update korisnik set pokusaji_logiranja = 3 where id = {$korisnikID}"; 
        $rezultat = $veza->updateDB($upit);
        
        $veza->zatvoriDB();

        $dnevnik = new Dnevnik();
        $dnevnik->zapisi("Administator blokira korisnika (ID korisnika: $korisnikID).", $_SESSION['id']);
        
        header("Location: PopisKorisnika.php?rezultat={$rezultat}");
    }
    
    // Postavljanje moderatora
    if(isset($_GET['korisnikModeratorID'])){
        $veza = new Baza();
        $veza->spojiDB();
        
        $korisnikID = $_GET['korisnikModeratorID'];
            
        $upit = "update korisnik set uloga_id = 2 where id = {$korisnikID}"; 
        $rezultat = $veza->updateDB($upit);
        
        $veza->zatvoriDB();
        
        $dnevnik = new Dnevnik();
        $dnevnik->zapisi("Administator daje korisniku (ID korisnika: $korisnikID) ulogu moderatora .", $_SESSION['id']);

        header("Location: PopisKorisnika.php?rezultat={$rezultat}");
    }
    
    // Uklanjanje moderatora
    if(isset($_GET['ukloniModeratoraID'])){
        $veza = new Baza();
        $veza->spojiDB();
        
        $korisnikID = $_GET['ukloniModeratoraID'];
            
        $upit = "update korisnik set uloga_id = 3 where id = {$korisnikID}"; 
        $rezultat = $veza->updateDB($upit);
        
        $veza->zatvoriDB();
        
        $dnevnik = new Dnevnik();
        $dnevnik->zapisi("Administator miče korisniku (ID korisnika: $korisnikID) ulogu moderatora .", $_SESSION['id']);
            ...
            <div class="odjeljak">
                ...
                <?php 
                    if(isset($_GET['rezultat'])){
                            if($_GET['rezultat'] == '1'){
                                echo '<p style="color:green"> Podaci su uspješno ažurirani.</p>';
                                header( "refresh:4;url=PopisKorisnika.php" );
                            }
                        }
                ?>
                ...
                <?php
                    ...
                    foreach ($nizZapisa as $value) {
                        ...
                        echo  "<td> {$value['id']} </td>"
                            ...
                            . "<td> {$value['email']} </td>";
                            if(intval($value['pokusaji_logiranja']) < 3){
                                echo "<td> <button type='button' onclick=location.href='PopisKorisnika.php?korisnikID={$value['id']}'> Blokiraj </button></td>";
                            }
                            else echo "<td> Već blokiran </td>";
                            if(intval($value['uloga_id']) > 2){
                                echo "<td> <button type='button' onclick=location.href='PopisKorisnika.php?korisnikModeratorID={$value['id']}'> Napravi moderatorom </button></td>";
                            }
                            else{
                                echo "<td> <button type='button' onclick=location.href='PopisKorisnika.php?ukloniModeratoraID={$value['id']}'> Ukloni moderatora </button></td>";
                            }
