<?php
    ...
    if(isset($_GET['obrisiID'])){
        $veza = new Baza();
        $veza->spojiDB();
        $obrisiID = intval($_GET['obrisiID']);
        $upit = "delete from licenca where id = {$obrisiID}";
        $rezultat = $veza->selectDB($upit);

        $veza->zatvoriDB();
        
        $dnevnik = new Dnevnik();
        $dnevnik->zapisi("Administrator briše licencu (ID licence: $obrisiID).", $_SESSION['id']);
...
<html lang="hr" >
    ...
    <header id="headerObrazac">
	   <div class = "container">
            <h2> <span class = "zutiTekst"> Pregled licenci </span></h2>
            <nav>
                <ul>
    ...
    <section style="text-align:center">
            <div class="odjeljak">
                <h3> Popis svih licenci </h3>
                <?php 
                    if(isset($_GET['rezultat'])){
                            if($_GET['rezultat'] == '1'){
                                echo '<p style="color:green"> Podaci su uspješno dodani/ažurirani/obrisani u bazi podataka.</p>';
                ...
                <?php
                    ...
                    foreach ($nizZapisa as $value) {
                        ...
                        echo  "<td> {$value['id']} </td>"
                            ...
                            . "<td> <button type='button' onclick=location.href='LicencaUredi.php?licencaID={$value['id']}'> Uredi </button> </td>"
                            . "<td> <button type='button' onclick=location.href='LicencaPopis.php?obrisiID={$value['id']}'> Obrisi </button> </td>";  
                        echo '</tr>';
                    }                
                ?>
                </table>
